...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      console.error(
        "Event edit form error:",
        err
      );

      res.status(500).send(
        "Unable to load the event edit form."
      );
    }
  }
);
      ...
      if (!title || !eventType || !team || !date || !time) {
        return res.status(400).render(
          "events/edit",
          {
            ...
            teams,
            error:
              "Title, event type, team, date, and time are required."
          }
        );
      }
